...
class Dispatcher:
    def __init__(self, state_delay_ms: int):
        self.seq = 1 # Serial number for outgoing packets
        self.waiting = {} # Waiting for ack
        self.retry_int = max(1, state_delay_ms // 10) / 1000.0 # retry interval in seconds
        ...
        for key, (payload, deadline, left) in list(self.waiting.items()):
            if now >= deadline:
                if left <= 0:
                    ...
                    yield key, payload # (addr, seq), payload to retry with generator