...
def to_proto_state(engine, players_list):
    ...
    for pid, s in engine.snakes.items():
        ...
        # Protobuf snake object connected with pid
        snake_pb = pb.GameState.Snake()
        snake_pb.player_id = pid
        pts = []

        if s.cells:
            pts.append(pb.GameState.Coord(x=s.cells[0].x, y=s.cells[0].y)) # Head
        ...
        # Packing protobuf snake
        snake_pb.points.extend(pts)
        snake_pb.state = pb.GameState.Snake.ALIVE
