...
void CSVWriter::write_to_file(const std::string& output_file_name, const std::map<std::string, int>& words_map)
{
    ...
    std::vector<std::tuple<std::string, int, float>> words_frequency(words_map.size());

    for (const auto& [word, count] : words_map)
    {
        float percent = (static_cast<float>(count) / static_cast<float>(words_map.size())) * 100;
